...
const emptyBackfill = (): BackfillRow => ({ date: today(), start: '09:00', end: '10:00', device: 'Laptop', application: '', category: 'Other' });
...
export default function TrackConsole() {
  ...
  const updateBackfill = (index: number, key: keyof BackfillRow, value: string) => setBackfillRows((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: value } : row));
  const saveBackfill = () => {
  ...
  return <div className="console-page">
    ...
    {modal && <div className="modal-backdrop"><div className="modal-card"><div className="modal-header"><h2>{modal === 'backfill' ? 'Add Past Activity · Quick Backfill' : 'Add Daily Estimate'}</h2><button className="icon-button" onClick={() => setModal(null)} aria-label="Close">×</button></div>{modal === 'backfill' ? <><p className="muted">Add activity you remember from before Lemon Tracking was running. These entries are marked Manual and keep their own timestamps.</p><table className="plain-table console-table"><thead><tr><th>Date</th><th>Start</th><th>End</th><th>Device</th><th>Application</th><th>Category</th><th /></tr></thead><tbody>{backfillRows.map((row, index) => <tr key={index}><td><input type="date" value={row.date} onChange={(event) => updateBackfill(index, 'date', event.target.value)} /></td><td><input type="time" value={row.start} onChange={(event) => updateBackfill(index, 'start', event.target.value)} /></td><td><input type="time" value={row.end} onChange={(event) => updateBackfill(index, 'end', event.target.value)} /></td><td><select value={row.device} onChange={(event) => updateBackfill(index, 'device', event.target.value)}>{DEVICES.map((device) => <option key={device}>{device}</option>)}</select></td><td><input placeholder="e.g. Canvas" value={row.application} onChange={(event) => updateBackfill(index, 'application', event.target.value)} /></td><td><select value={row.category} onChange={(event) => updateBackfill(index, 'category', event.target.value)}>{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></td><td><button className="button button-danger button-small" onClick={() => setBackfillRows((rows) => rows.length === 1 ? rows : rows.filter((_, rowIndex) => rowIndex !== index))}>Remove</button></td></tr>)}</tbody></table><div className="form-actions"><button className="button button-small" onClick={() => setBackfillRows((rows) => [...rows, emptyBackfill()])}>Add row</button><button className="button button-small" onClick={() => setBackfillRows((rows) => [...rows, { ...(rows[rows.length - 1] ?? emptyBackfill()) }])}>Duplicate last row</button><button className="button button-primary button-small" onClick={saveBackfill}>Save day</button></div></> : <><p className="muted">Use durations when exact timestamps are not known. Lemon Tracking stores these as aggregate Estimated records and does not invent a clock position.</p><div className="form-grid"><div className="form-field"><label>Date<input type="date" value={estimateForm.date} onChange={(event) => setEstimateForm({ ...estimateForm, date: event.target.value })} /></label></div><div className="form-field"><label>Optional device estimate<select value={estimateForm.device} onChange={(event) => setEstimateForm({ ...estimateForm, device: event.target.value })}>{DEVICES.map((device) => <option key={device}>{device}</option>)}</select></label></div>{[['education', 'Education & Learning'], ['social', 'Social & Communication'], ['gaming', 'Gaming'], ['entertainment', 'Passive Entertainment'], ['work', 'Work / Productivity'], ['other', 'Other']].map(([key, label]) => <div className="form-field" key={key}><label>{label}<input type="number" min="0" step="1" value={estimateForm[key as keyof typeof estimateForm] as string} onChange={(event) => setEstimateForm({ ...estimateForm, [key]: event.target.value })} placeholder="minutes" /></label></div>)}<div className="form-field"><label>Confidence<select value={estimateForm.confidence} onChange={(event) => setEstimateForm({ ...estimateForm, confidence: event.target.value as Confidence })}><option>Exact</option><option>Approximate</option><option>Rough estimate</option></select></label></div></div><div className="form-actions"><button className="button button-primary button-small" onClick={saveEstimate}>Save estimate</button></div></>}</div></div>}
