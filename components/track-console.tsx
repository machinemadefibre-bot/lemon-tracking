...
type TimelineRange = 'today' | 'yesterday' | 'date' | 'week';
...
const shiftDate = (value: string, days: number) => { const date = new Date(`${value}T12:00:00`); date.setDate(date.getDate() + days); return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; };
const startOfWeek = (value: string) => { const date = new Date(`${value}T12:00:00`); const offset = (date.getDay() + 6) % 7; return shiftDate(value, -offset); };
const addMinutesToTime = (value: string, minutes: number) => { const base = timeMinutes(value) ?? 0; const total = (base + Math.max(0, Math.round(minutes))) % 1440; return `${String(Math.floor(total / 60)).padStart(2, '0')}:${String(total % 60).padStart(2, '0')}`; };
const unionMinutesFor = (entries: Entry[]) => {
  ...
  for (const interval of intervals) {
    if (currentStart === undefined || currentEnd === undefined) { currentStart = interval.start; currentEnd = interval.end; continue; }
    if (interval.start > currentEnd) { total += currentEnd - currentStart; currentStart = interval.start; currentEnd = interval.end; }
    else currentEnd = Math.max(currentEnd, interval.end);
  }
  return currentStart === undefined || currentEnd === undefined ? total : total + currentEnd - currentStart;
};
const emptyBackfill = (): BackfillRow => ({ id: makeId(), date: today(), start: '09:00', end: '10:00', durationMinutes: '60', device: 'Laptop', application: '', category: 'Other' });
...
export default function TrackConsole() {
  ...
  const [backfillBatchDate, setBackfillBatchDate] = useState(today());
  ...
  const timelineEntries = useMemo(() => {
    ...
    if (timelineRange === 'week') {
      ...
      return filteredEntries.filter((entry) => entry.date >= weekStart && entry.date <= weekEnd);
    }
    const date = timelineRange === 'today' ? currentDate : timelineRange === 'yesterday' ? shiftDate(currentDate, -1) : selectedDate;
    return filteredEntries.filter((entry) => entry.date === date);
  }, [filteredEntries, selectedDate, timelineRange]);
  ...
  const mostUsedApplication = Array.from(new Set(activeEntries.map((entry) => entry.application).filter(Boolean))).map((application) => ({ application, minutes: activeEntries.filter((entry) => entry.application === application).reduce((sum, entry) => sum + entry.durationMinutes, 0) })).sort((a, b) => b.minutes - a.minutes)[0]?.application ?? '—';
  const mostUsedCategory = Array.from(new Set(activeEntries.map((entry) => entry.category).filter(Boolean))).map((category) => ({ category, minutes: activeEntries.filter((entry) => entry.category === category).reduce((sum, entry) => sum + entry.durationMinutes, 0) })).sort((a, b) => b.minutes - a.minutes)[0]?.category ?? '—';
  ...
  const todayCategoryTotals = useMemo(() => CATEGORIES.map((category) => ({ category, minutes: todayEntries.filter((entry) => entry.category === category).reduce((sum, entry) => sum + entry.durationMinutes, 0) })), [todayEntries]);
  const maxTodayCategory = Math.max(1, ...todayCategoryTotals.map((row) => row.minutes));
  const rawDeviceMinutes = timelineEntries.reduce((sum, entry) => sum + entry.durationMinutes, 0);
  const unionMinutes = unionMinutesFor(timelineEntries);

  const showToast = (message: string) => setToast(message);
  const openBackfill = (rows: BackfillRow[] = [emptyBackfill()]) => { setBackfillRows(rows); setBackfillBatchDate(rows[0]?.date ?? today()); setCopyDates(''); setModal('backfill'); };
  ...
  const updateBackfill = (id: string, key: Exclude<keyof BackfillRow, 'id' | 'durationMinutes'>, value: string) => setBackfillRows((rows) => rows.map((row) => {
    if (row.id !== id) return row;
    const next = { ...row, [key]: value };
    if (key === 'start' || key === 'end') next.durationMinutes = String(durationBetween(next.start, next.end));
    return next;
  }));
  const updateBackfillDuration = (id: string, value: string) => setBackfillRows((rows) => rows.map((row) => row.id === id ? { ...row, durationMinutes: value, end: addMinutesToTime(row.start, Number(value) || 0) } : row));
  const saveBackfill = () => {
    const created = backfillRows.filter((row) => row.application.trim() && Number(row.durationMinutes) > 0).map((row): Entry => ({ id: makeId(), date: row.date, start: row.start, end: row.end, durationMinutes: Number(row.durationMinutes), application: row.application.trim(), category: row.category, device: row.device, source: 'manual', createdAt: now(), modifiedAt: now() }));
  ...
  const duplicateBackfillRow = (id: string) => setBackfillRows((rows) => { const index = rows.findIndex((row) => row.id === id); if (index < 0) return rows; const copy = { ...rows[index], id: makeId() }; return [...rows.slice(0, index + 1), copy, ...rows.slice(index + 1)]; });
  const parseCopyDates = () => Array.from(new Set(copyDates.split(/[\s,]+/).map((value) => value.trim()).filter((value) => /^\d{4}-\d{2}-\d{2}$/.test(value))));
  const copyBackfillToDates = (rows: BackfillRow[]) => { const dates = parseCopyDates(); if (!dates.length) { showToast('Enter one or more dates in YYYY-MM-DD format.'); return; } setBackfillRows((current) => [...current, ...dates.flatMap((date) => rows.map((row) => ({ ...row, id: makeId(), date })))]); showToast(`${rows.length} activity row${rows.length === 1 ? '' : 's'} copied to ${dates.length} date${dates.length === 1 ? '' : 's'}.`); };
  const applyBackfillBatchDate = () => { if (!/^\d{4}-\d{2}-\d{2}$/.test(backfillBatchDate)) return; setBackfillRows((rows) => rows.map((row) => ({ ...row, date: backfillBatchDate }))); showToast('Dates updated for the backfill rows.'); };
  const saveEstimate = () => {
  ...
  const renderHeaderActions = () => <div className="console-actions"><button className="button button-primary" onClick={() => openBackfill()}>Add past activity</button><button className="button" onClick={() => setModal('estimate')}>Daily estimate</button>{view === 'today' && <button className="button" onClick={pauseToggle}>{paused ? 'Resume tracking' : 'Pause tracking'}</button>}</div>;
  const renderFilter = (entriesInView = selectedEntries) => <div className="filter-row"><label className="field-inline">Data source<select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value as 'all' | Source)}><option value="all">All Activity</option><option value="automatic">Automatically Tracked</option><option value="manual">Manually Added</option><option value="estimated">Estimated</option><option value="imported">Imported</option></select></label><span className="status-text">{entriesInView.length} record{entriesInView.length === 1 ? '' : 's'} in view</span></div>;
  const renderToday = () => <>
    ...
    <div className="metric-grid"><div className="metric-card"><span>Screen activity</span><strong>{formatDuration(activeMinutes)}</strong><small>AFK excluded</small></div><div className="metric-card"><span>Active applications</span><strong>{appCount}</strong><small>today</small></div><div className="metric-card"><span>Longest session</span><strong>{formatDuration(longest)}</strong><small>observed or entered</small></div><div className="metric-card"><span>Idle time</span><strong>{formatDuration(idleMinutes)}</strong><small>separate from active</small></div><div className="metric-card"><span>Most-used application</span><strong className="metric-value-text">{mostUsedApplication}</strong><small>active records</small></div><div className="metric-card"><span>Most-used category</span><strong className="metric-value-text">{mostUsedCategory}</strong><small>active records</small></div></div>
    <section className="console-section"><h2>Categories</h2><div className="category-list">{todayCategoryTotals.map(({ category, minutes }) => <div className="category-row" key={category}><span>{category}</span><div className="category-track"><div className="category-fill" style={{ width: `${Math.round((minutes / maxTodayCategory) * 100)}%` }} /></div><span>{formatDuration(minutes)}</span></div>)}</div></section>
    <section className="console-section"><h2>Recent activity</h2>{renderFilter(todayEntries)}{todayEntries.length ? <table className="plain-table console-table"><thead><tr><th>Time</th><th>Application</th><th>Device</th><th>Duration</th><th>Source</th></tr></thead><tbody>{todayEntries.slice().sort((a, b) => (b.start ?? '').localeCompare(a.start ?? '')).slice(0, 12).map((entry) => <tr key={entry.id}><td>{entry.start ? `${entry.start}–${entry.end}` : 'Aggregate'}</td><td>{entry.application}</td><td>{entry.device}</td><td>{formatDuration(entry.durationMinutes)}</td><td><SourceChip source={entry.source} /></td></tr>)}</tbody></table> : <EmptyState onAdd={() => openBackfill()} />}</section>
  </>;

  const renderTimeline = () => {
    const devices = Array.from(new Set(timelineEntries.map((entry) => entry.device)));
    const weekStart = startOfWeek(today());
    const weekEnd = shiftDate(weekStart, 6);
    const selectedLabel = timelineRange === 'today' ? `Today · ${formatDate(today())}` : timelineRange === 'yesterday' ? `Yesterday · ${formatDate(shiftDate(today(), -1))}` : timelineRange === 'week' ? `Week · ${formatDate(weekStart)}–${formatDate(weekEnd)}` : formatDate(selectedDate);
    return <><div className="filter-row"><div className="timeline-range-actions"><button className={`button button-small ${timelineRange === 'today' ? 'button-selected' : ''}`} onClick={() => setTimelineRange('today')}>Today</button><button className={`button button-small ${timelineRange === 'yesterday' ? 'button-selected' : ''}`} onClick={() => setTimelineRange('yesterday')}>Yesterday</button><button className={`button button-small ${timelineRange === 'week' ? 'button-selected' : ''}`} onClick={() => setTimelineRange('week')}>Week View</button><label className="field-inline">Date<input type="date" value={selectedDate} onChange={(event) => { setSelectedDate(event.target.value); setTimelineRange('date'); }} /></label></div>{renderFilter(timelineEntries)}</div><p className="status-text">{selectedLabel}</p><div className="metric-grid"><div className="metric-card"><span>Raw device activity</span><strong>{formatDuration(rawDeviceMinutes)}</strong><small>sum of device intervals</small></div><div className="metric-card"><span>Clock time with activity</span><strong>{formatDuration(unionMinutes)}</strong><small>overlap counted once</small></div><div className="metric-card"><span>Overlap</span><strong>{formatDuration(Math.max(0, rawDeviceMinutes - unionMinutes))}</strong><small>shown by device row</small></div></div><section className="console-section"><h2>{timelineRange === 'week' ? 'Weekly activity' : 'Clock-time timeline'}</h2>{timelineRange === 'week' ? <table className="plain-table console-table"><thead><tr><th>Date</th><th>Device</th><th>Application</th><th>Time</th><th>Duration</th><th>Source</th></tr></thead><tbody>{timelineEntries.slice().sort((a, b) => `${b.date}${b.start ?? ''}`.localeCompare(`${a.date}${a.start ?? ''}`)).map((entry) => <tr key={entry.id}><td>{formatDate(entry.date)}</td><td>{entry.device}</td><td>{entry.application}</td><td>{entry.start ? `${entry.start}–${entry.end}` : 'Aggregate'}</td><td>{formatDuration(entry.durationMinutes)}</td><td><SourceChip source={entry.source} /></td></tr>)}</tbody></table> : devices.length ? <div className="timeline">{devices.map((device) => <div className="timeline-row" key={device}><span className="timeline-label">{device}</span><div className="timeline-track">{timelineEntries.filter((entry) => entry.device === device && entry.start && entry.end && !entry.afk).map((entry) => { const start = timeMinutes(entry.start) ?? 0; const width = Math.max(1, entry.durationMinutes) / 1440 * 100; return <div className={`timeline-event ${entry.source === 'manual' ? 'manual' : entry.source === 'estimated' ? 'estimate' : ''}`} key={entry.id} style={{ left: `${start / 1440 * 100}%`, width: `${width}%` }} title={`${entry.application} · ${formatDuration(entry.durationMinutes)} · ${labelSource(entry.source)}`}>{entry.application}</div>; })}</div></div>)}</div> : <EmptyState onAdd={() => openBackfill()} />}</section><p className="status-text" style={{ marginTop: 15 }}>Timeline scale: 00:00 · 04:48 · 09:36 · 14:24 · 19:12 · 24:00. Aggregate estimates appear in history without fabricated clock positions.</p></>;
  };

  const renderApplications = () => <><div className="console-section"><h2>Applications</h2>{renderFilter()}{visibleApps.length ? <div className="app-list">{visibleApps.map((item) => <div className="app-list-row" key={item.application}><strong>{item.application}</strong><span>{formatDuration(item.minutes)}</span><select value={item.category} onChange={(event) => updateApplicationCategory(item.application, event.target.value)} aria-label={`Category for ${item.application}`}>{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></div>)}</div> : <EmptyState onAdd={() => openBackfill()} />}</div></>;
