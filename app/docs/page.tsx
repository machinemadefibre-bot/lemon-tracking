...
export default function DocsPage() {
  return (
    <SiteShell current="Documentation">
      <h1>Documentation</h1>
      <p>Operating notes for the 0.1.0 desktop collectors, Android application and local console.</p>
      <h2>First run</h2>
      ...
      <table className="plain-table feature-table"><thead><tr><th>Platform</th><th>Collector</th><th>Foreground adapter</th><th>Idle adapter</th></tr></thead><tbody><tr><td>Windows x64</td><td>Desktop collector</td><td>Win32 foreground window and process identity</td><td>Windows last-input timer</td></tr><tr><td>macOS Apple Silicon</td><td>Local agent</td><td>System Events through osascript</td><td>IOHIDSystem through ioreg</td></tr><tr><td>Linux x86_64</td><td>Local agent</td><td>X11 xprop</td><td>xprintidle when installed</td></tr><tr><td>Android</td><td>APK</td><td>UsageStatsManager through Usage Access</td><td>Current-day foreground summary</td></tr></tbody></table>
      <p>Android: open Lemon Tracking. If the device displays a restricted-setting message, open <strong>App info</strong> and select <strong>Allow restricted settings</strong>. Then open <strong>Usage Access</strong>, enable access in system settings, return to the app and select <strong>Refresh today</strong>.</p>
      <h2>Local files</h2>
      <p>Each collector uses the current user&apos;s local application-data area:</p><div className="plain-code">LemonTracking/</div><p style={{ marginTop: 12 }}>The folder contains consent state and activity records. Windows Settings opens the folder; the macOS and Linux agent prints the path at startup. Android stores <code>records.json</code> in the application&apos;s private files directory.</p>
      <h2>Data path</h2>
